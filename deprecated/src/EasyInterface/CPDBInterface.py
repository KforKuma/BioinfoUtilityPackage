...
def extract_cpdb_result(file_dir):
    cpdb_results = {
        'deconvoluted_percents': pd.read_table(
            file_dir + [s for s in os.listdir(file_dir) if bool(re.search("analysis_deconvoluted_percents", s))][0],
            delimiter='\t'),
        'deconvoluted': pd.read_table(
            file_dir + [s for s in os.listdir(file_dir) if bool(re.search("analysis_deconvoluted", s))][0],
            delimiter='\t'),
        'means': pd.read_table(file_dir + [s for s in os.listdir(file_dir) if bool(re.search("analysis_means", s))][0],
                               delimiter='\t'),
        'pvalues': pd.read_table(file_dir + [s for s in os.listdir(file_dir) if
                                             bool(re.search("analysis_pvalues|analysis_relevant_interactions", s))][0],
                                 delimiter='\t'),
        'interaction_scores': pd.read_table(
            file_dir + [s for s in os.listdir(file_dir) if bool(re.search("analysis_interaction_scores", s))][0],
            delimiter='\t'),
        'significant_means': pd.read_table(
            file_dir + [s for s in os.listdir(file_dir) if bool(re.search("analysis_significant_means", s))][0],
            delimiter='\t')
...
# for i, var in enumerate([TNFsf_1,TNFsf_2,TNFsf_3], start=1):
#     # 使用 locals() 函数获取当前作用域的所有变量
#     var_name = [name for name, value in locals().items() if value is var][0]
#     print(f'变量名为 "{var_name}"，值为 "{var}"')
#
...
def extract_cpdb_table(
    ...
    col_start = (
        ...
        # 魔法数字，把行列数硬编码进去；在cellphonedb v5版本, 具体的数值前有12列meta信息列；因此col_start是13
    ...
    if splitby_key is not None:
        ...
        # 构造“_labels”列
        metadata["_labels"] = [split + "_" + celltype for split, celltype in zip(metadata[splitby_key], metadata[celltype_key])]
