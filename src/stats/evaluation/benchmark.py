...
from src.stats.evaluation.evaluator import EvaluationSpec, evaluate_contrasts
from src.stats.pipeline import _build_adapters, _canonical_from_simulation, _decision_registry
from src.stats.pipeline import DifferentialAbundanceRunner
...
PROJECT_CONFIG_ROOT = REPOSITORY_ROOT / "project" / "Step08_Abundance" / "configs"
DEFAULT_REGISTRY = PROJECT_CONFIG_ROOT / "method_benchmark_registry.yaml"
DEFAULT_SCENARIOS = PROJECT_CONFIG_ROOT / "phase5_scenario_matrix.csv"
DEFAULT_EVALUATION = PROJECT_CONFIG_ROOT / "phase5_evaluation_spec.yaml"
DEFAULT_CONFIG = PROJECT_CONFIG_ROOT / "phase5_benchmark.yaml"
...
def derive_expanded_benchmark(
    ...
    *,
    output_base: str | Path = REPOSITORY_ROOT / "benchmark_runs",
    registry_path: str | Path = PROJECT_CONFIG_ROOT / "phase6_method_benchmark_registry.yaml",
    config_path: str | Path = PROJECT_CONFIG_ROOT / "phase6_benchmark.yaml",
) -> Path:
...
def main(argv: list[str] | None = None) -> None:
    ...
    derive.add_argument(
        "--registry", type=Path,
        default=PROJECT_CONFIG_ROOT / "phase6_method_benchmark_registry.yaml",
    )
    derive.add_argument(
        "--config", type=Path,
        default=PROJECT_CONFIG_ROOT / "phase6_benchmark.yaml",
    )
