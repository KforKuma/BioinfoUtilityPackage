...
def plot_simulation_benchmarks(df, save_addr, filename):
    ...
    for ax, factor in zip(axes, factors):
        ...
        # --- 绘制 FPR (右轴) ---
        p2 = sns.lineplot(data=sub_df, x='scale_factor', y='FPR', hue='Method',
                          marker='X', ax=ax2, legend=False, linestyle='--', alpha=0.6)
        ax2.set_ylabel('FPR (1 - Specificity)', fontsize=12, color='red')
        ax2.set_ylim(-0.05, 1.05)
        
        # 绘制 FPR = 0.05 的参考线
        ax2.axhline(0.05, color='red', linestyle=':', alpha=0.5, label='Target FPR (0.05)')
...
def plot_simulation_with_inflam_marginalized(
    ...
    plot_df["FPR"] = pd.to_numeric(plot_df["FPR"])
    ...
    axes[1].set_title(f"FPR (Marginalized over Inflam, {ci_type})", fontsize=14)
    axes[1].set_ylim(0, 1.05)
...
def plot_ratio_scatter(
    ...
    categories = plot_df[disease_col].unique()
    ...
    for cat in categories:
        ...
        xs = xpos[cat] + np.random.uniform(-jitter, jitter, size=len(sub))
        ax.scatter(xs, sub[y_col], alpha=0.8)
    ...
    if clr_lmm_result is not None:
        ...
        for other, row in ct.iterrows():
            ...
            ax.text((x1 + x2) / 2, level + h * 1.2,
                    star, ha="center", va="bottom")
            
            level += h * 2
    
    fig.savefig(f"{save_addr}/{filename}.png", dpi=300)
    fig.savefig(f"{save_addr}/{filename}.pdf")
...
def plot_stacked_barplot(
    ...
    # 计算比例（避免 merge）
    total_counts = df.groupby(by_col)["count"].transform("sum")
    df["prop"] = df["count"] / total_counts
    
    # pivot：sample × cell_type
